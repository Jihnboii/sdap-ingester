...
granule_events = defaultdict(list)  # Tracks granule events by dataset ID
        ...
        for collection in self._collections_for_dir:
            try:
                if collection.owns_file(path):
                    ...
                    # Log the granule event
                    logger.info(f"Granule event detected: Path={path}, ModifiedTime={modified_time}, DatasetID={collection.dataset_id}")

                    # Track the granule event
                    granule_events[collection.dataset_id].append({
                        "path": path,
